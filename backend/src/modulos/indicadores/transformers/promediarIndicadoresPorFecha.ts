...
export const indicadoresCalculadosPorfecha = (responseData: {
    [key: string]: IndicadorCalculado[];
}, indicatorMapping: Indicador[]): IndicadorCalcularPeriodo[] => {
    ...
    return dates.map(dateStr => {
        ...
        responseData[dateStr].forEach(indicadorCalculado => {
            const indicador = indicatorMapping.find(i => i.id === indicadorCalculado.idIndicador);

            if (indicador) {
                // Convertir el valor a porcentaje (multiplicar por 100) y redondear a 2 decimales
                result[indicador.nombre] = Math.round(indicadorCalculado.valor * 100 * 100) / 100;
            } else {
                console.log("indicador no encontrado", indicadorCalculado.idIndicador, indicatorMapping.length);
            }
        });
...
export const indicadoresCalculadosPorPeriodo = (data: IndicadorCalcularPeriodo[], inicio: string, fin: string): IndicadorCalcularPeriodo[] => {
...
export const indicadoresCalculadosPorMes = (data: IndicadorCalcularPeriodo[], indicadores: Indicador[]): IndicadorCalcularPeriodo[] => {
